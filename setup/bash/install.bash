...
CONTENT="source \"$TOOLS_ROOT/setup/bash/setup.bash\""
...
ensure_bash_command_permissions() {
  chmod u+x \
    "$TOOLS_ROOT/scripts/bash/rdt" \
    "$TOOLS_ROOT/scripts/bash/rmvl" \
    "$TOOLS_ROOT/scripts/bash/lpss" \
    "$TOOLS_ROOT/scripts/bash/lviz" \
    "$TOOLS_ROOT/scripts/bash/.rdt/"*.bash \
    "$TOOLS_ROOT/scripts/bash/.lpss/"*.bash
}
...
if [ -f "$BASHRC" ] && grep -qF "$MARKER_START" "$BASHRC"; then
  ...
  existing_root_path=${existing_root_path%$'\r'}
  ...
  # Always refresh the managed block so upgrades can migrate setup paths and exports.
...
detect_rmvl_cxx_standard() {
  ...
  version_file="$(find "$build_dir" -type f -name version_string.inc -print -quit)"
  if [ -z "$version_file" ]; then
    log_error "无法从 RMVL 构建目录读取 C++ 标准: 未找到 version_string.inc"
    return 1
  fi

  standard="$(sed -nE 's/.*C\+\+ standard:[[:space:]]*([0-9]+).*/\1/p' "$version_file" | head -n1)"
  if [[ ! "$standard" =~ ^[0-9]+$ ]]; then
    log_error "无法从 RMVL 构建信息中解析 C++ 标准: $version_file"
    return 1
  fi

  RDT_CXX_STANDARD="$standard"
}
...
RDT_CXX_STANDARD=""
detect_rmvl_cxx_standard "$build_ws"
log_info "检测到 RMVL C++ 标准: C++$RDT_CXX_STANDARD"

unset cur_dir build_ws

log_info "正在构建 rdt..."
run_cmd cmake -S "$TOOLS_ROOT/src" -B "$TOOLS_ROOT/build_tmp" "-DCMAKE_CXX_STANDARD=$RDT_CXX_STANDARD"
run_cmd cmake --build "$TOOLS_ROOT/build_tmp"
log_success "rdt 构建完成"

for name in tool viz; do
  cp "$TOOLS_ROOT/build_tmp/lpss_$name" "$TOOLS_ROOT/scripts/bash/.lpss/_autogen_lpss_$name"
  chmod u+x "$TOOLS_ROOT/scripts/bash/.lpss/_autogen_lpss_$name"
done
